...
## 1. 프로젝트 개요 (Overview)
**Transfy**는 스포티파이(Spotify)에서 현재 재생 중인 음악의 가사를 실시간으로 가져와, 사용자가 선택한 언어로 번역하여 싱크(Sync)에 맞춰 보여주는 웹 애플리케이션입니다.

- **목표**: 언어 장벽 없이 음악을 즐길 수 있는 경험 제공
- **주요 기능**:
    - 스포티파이 실시간 재생 정보 연동 (Polling)
...
- **Auth**: NextAuth.js (Spotify, Google Provider)
- **External APIs**:
...
1. **인증 (Auth)**: 사용자가 `NextAuth`를 통해 스포티파이 계정으로 로그인합니다.
2. **폴링 (Polling)**: 클라이언트(`useSpotifyPoller`)가 1초(또는 가변적)마다 스포티파이 API를 호출하여 현재 재생 중인 트랙과 진행 시간(`progress_ms`)을 가져옵니다.
